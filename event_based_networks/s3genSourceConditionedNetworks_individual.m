...
for r1 = [1:64]
    ...
    npix{r1} = numpix(r1); %need to recast as cell because of matlab table 6__6
        ...
        spatialmapname = [expLoc '/results/conditionalsinks/individualmatfiles/iexp=' num2str(iexp) '/' conds{condsindexlist(1)} '/' 'thresh=' num2str(thresh) '/'];
        ...
        if exist(spatialmapname)
            ...
            [num2str(iexp) ' ' num2str(r1) ' here1']
            continue
        end
        
        if numpix(r1) < 50;
            [num2str(iexp) num2str(r1) ' here2']
            continue
        end
        ...
        for i = 1:1:max(max(mask));
            ...
            Adj_term(r1, i) = Adj_term(r1, i) + (nansum(nansum(spatialmap_term(:,:).*temp)));
            
            %Weights are proportional to the number of avalanches
            Adj_act(r1, i) = Adj_act(r1, i) + (nansum(nansum(spatialmap_act(:,:).*temp)));


        end
        numActivations{r1} = numActivations{r1} + numavs;
        
    end


G_term = digraph(Adj_term, labels);
G_act = digraph(Adj_act, labels);


savedir = ['/home/dcuric/Documents/sourcesinksanalysis' '/results/graphs/individuals/' conds{condsindexlist(1)} '/iexp=' num2str(iexp) '/' ];
...
clear Adj_term Adj_act numActivations
  end
